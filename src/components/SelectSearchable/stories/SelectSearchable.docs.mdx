...
Headers and dividers are evaluated after duplicates are removed, so sections with no
remaining options are hidden. Prefer stable database IDs as values, and keep React
...
- No special identity prop is required. `OptionList` assigns structural ids internally.

  ### Identity requirements

  - React keys should be unique and stable when mapping arrays.
  - Duplicate option values use the OptionList first-wins policy, with or without keys.
  - Duplicate structural row identities (headers/dividers) remain invalid.
...
- Headers are identified internally by `OptionList`.
...
- Dividers are identified internally by `OptionList`.
